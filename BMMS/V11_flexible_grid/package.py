"""Package the independently checked R11 descendants and an identical control."""
...
def main():
    ...
    for version,run in checks['runs'].items():
        assert run['source_runs']==464 and run['repeat_pairs']==232
    ...
    assert grid==checks['grid_checks'] and grid['checked_shapes']==8272 and grid['changed']=={'R13':1112,'R14':34}
    ...
    assert {p.name for p in files}=={'README.md','MANIFEST.json','CPU_CHECKS.json','R11_CONTROL.asc','R13_FLEX_SHORT_WAVE.asc','R14_FLEX_SINGLE_WAVE.asc'}
    target=ROOT/'BMMS_V11_R13_R14_提交包.zip'
    with zipfile.ZipFile(target,'w',compression=zipfile.ZIP_DEFLATED) as z:
        for p in files:
            e=zipfile.ZipInfo('BMMS_V11_R13_R14/'+p.name,date_time=(2026,9,26,0,0,0))
            e.compress_type=zipfile.ZIP_DEFLATED;z.writestr(e,p.read_bytes())
    with zipfile.ZipFile(target) as z:
        assert z.testzip() is None
        for p in files:assert z.read('BMMS_V11_R13_R14/'+p.name)==p.read_bytes()
