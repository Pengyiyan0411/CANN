"""Freeze independent candidates together with the byte-identical R04 control."""
...
spec=importlib.util.spec_from_file_location('grid_packet_build',HERE/'build.py')
...
def main():
    ...
    for version,run in checks['runs'].items():
        assert run['source_runs']==356 and run['strict_misses']==run['combined_misses']==0
    ...
    assert {p.name for p in files}=={'README.md','MANIFEST.json','CPU_CHECKS.json','R04_CONTROL.asc',
        'R07_BALANCED_GRID.asc','R08_NATIVE_PACKETS.asc'}
    path=ROOT/'BMMS_V11_R07_R08_提交包.zip'
    with zipfile.ZipFile(path,'w',compression=zipfile.ZIP_DEFLATED) as z:
        for p in files:
            entry=zipfile.ZipInfo('BMMS_V11_R07_R08/'+p.name,date_time=(2026,9,26,0,0,0))
            entry.compress_type=zipfile.ZIP_DEFLATED;z.writestr(entry,p.read_bytes())
    with zipfile.ZipFile(path) as z:
        assert z.testzip() is None
        for p in files:assert z.read('BMMS_V11_R07_R08/'+p.name)==p.read_bytes()
    report={'file':path.name,'sha256':build.sha(path),'bytes':path.stat().st_size,'members':len(files),
            'R04_control_unchanged':True,'independent_R04_descendants':True,
