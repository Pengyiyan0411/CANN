"""Append Native candidates and diagnosis; preserve all 464 previous archive entries."""
...
def main():
    ...
    subprocess.run([sys.executable,str(ROOT/'V11_a_resident/archive.py'),'--repo',str(repo)],check=True)
    manifest=json.loads(path.read_text(encoding='utf-8'));entries={r['path']:r for r in manifest['files']};sources=[]
    for folder in ['V11_native_specialize','BMMS_V11_R20_R21']:
        sources.extend(p for p in (ROOT/folder).iterdir() if p.is_file())
    sources.extend(ROOT/p for p in ['BMMS_V11_R20_R21_提交包.zip','audit_current/AUDIT_R17_R18_R19_DELIVERY.md'])
    ...
    assert (out/'audit_current/AUDIT_R17_R18_R19_DELIVERY.md').read_bytes()==frozen
    manifest['scope']='Frozen V9/V10/V11 history; R14 baseline; user Native Case 5 diagnosis; independent R20 wide-N MMAD and R21 L0 reuse candidates, awaiting platform validation'
    ...
    shutil.copyfile(ROOT/'V11_native_specialize/CANN_README.md',repo/'README.md')
    ignore=repo/'.gitignore';text=ignore.read_text(encoding='utf-8')
    for folder in ['cpu_build','dispatch_build']:
        pattern='BMMS/V11_native_specialize/'+folder+'/'
        if pattern not in text:text+=pattern+'\n'
