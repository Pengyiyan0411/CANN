"""Package exact, independently verified sources and their evidence."""
from pathlib import Path
import ast,importlib.util,json,zipfile
HERE=Path(__file__).resolve().parent;ROOT=HERE.parent
spec=importlib.util.spec_from_file_location('builder',HERE/'build.py');build=importlib.util.module_from_spec(spec);spec.loader.exec_module(build)

def main():
    build.verify();manifest=json.loads((build.OUT/'MANIFEST.json').read_text(encoding='utf-8'))
    checks=json.loads((HERE/'CHECKS.json').read_text(encoding='utf-8'))
    assert checks==json.loads((build.OUT/'CPU_CHECKS.json').read_text(encoding='utf-8'))
    for row in [manifest['control'],*manifest['variants']]:
        assert build.sha(build.OUT/row['file'])==row['sha256']==checks['sources'][row['file']]
    for rel,digest in checks['artifacts'].items():assert build.sha(ROOT/rel)==digest,rel
    results=[]
    for version in ['R14','R20','R21']:
        run=checks['runs'][version];assert run['source_runs']==624 and run['repeat_pairs']==312
        assert run['strict_misses']==run['combined_misses']==0 and run['known_precision_limitations']==3
        actual=json.loads((HERE/(version+'_CPU_RESULT.json')).read_text(encoding='utf-8'))
        assert {k:v for k,v in actual.items() if k!='output_bits'}==run
        results.append(actual['output_bits'])
    assert results[0]==results[1]==results[2] and all(v['rejected'] for v in checks['negative_controls'].values())
    dispatch=json.loads((HERE/'DISPATCH_CHECKS.json').read_text(encoding='utf-8'))
    for rel,digest in dispatch['sources'].items():assert build.sha(ROOT/rel)==digest
    assert dispatch['actual_entry_checks']==99840
    for p in HERE.glob('*.py'):ast.parse(p.read_text(encoding='utf-8'),filename=p.name)
    files=sorted(p for p in build.OUT.iterdir() if p.is_file())
    assert {p.name for p in files}=={'README.md','MANIFEST.json','CPU_CHECKS.json','R14_CONTROL.asc',*(v+'.asc' for v in build.NAMES.values())}
    target=ROOT/'BMMS_V11_R20_R21_提交包.zip'
    with zipfile.ZipFile(target,'w',compression=zipfile.ZIP_DEFLATED) as z:
        for p in files:
            item=zipfile.ZipInfo('BMMS_V11_R20_R21/'+p.name,date_time=(2026,9,27,0,0,0));item.compress_type=zipfile.ZIP_DEFLATED
            z.writestr(item,p.read_bytes())
    with zipfile.ZipFile(target) as z:
        assert z.testzip() is None
        for p in files:assert z.read('BMMS_V11_R20_R21/'+p.name)==p.read_bytes()
    report={'file':target.name,'sha256':build.sha(target),'bytes':target.stat().st_size,'members':len(files),
        'independent_R14_descendants':True,'R14_control_unchanged':True,'cann_compiled_locally':False,'npu_tested_locally':False}
    build.write(HERE/'PACKAGE_CHECKS.json',json.dumps(report,indent=2)+'\n');print(json.dumps(report,indent=2))

if __name__=='__main__':main()
