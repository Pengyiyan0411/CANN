"""Append R10 feedback and R11/R12 delivery, preserving all frozen prior evidence."""
...
PREVIOUS_ARCHIVE_COMMIT='44a67cc33ebdbee620904de5f03681a244216d5a'
def main():
    ...
    subprocess.run([sys.executable,str(ROOT/'V11_recovery/archive.py'),'--repo',str(repo)],check=True)
    manifest=json.loads(path.read_text(encoding='utf-8'));entries={r['path']:r for r in manifest['files']}
    sources=[]
    for folder in ['V11_single_wave','BMMS_V11_R11_R12']:
        sources.extend(p for p in (ROOT/folder).iterdir() if p.is_file())
    sources.extend(ROOT/p for p in ['BMMS_V11_R11_R12_提交包.zip','audit_current/AUDIT_R09_R10_DELIVERY.md'])
    ...
    assert (out/'audit_current/AUDIT_R09_R10_DELIVERY.md').read_bytes()==frozen
    manifest['scope']='V9/V10/V11 history; R10 user-reported stable benefit and 15-pass typical screenshot; independent R11 short-wave grid and R12 parallel N-merge candidates with frozen R10 control'
    ...
    shutil.copyfile(ROOT/'V11_single_wave/CANN_README.md',repo/'README.md')
    ignore=repo/'.gitignore';text=ignore.read_text(encoding='utf-8')
    if 'BMMS/V11_single_wave/cpu_build/' not in text:text+='BMMS/V11_single_wave/cpu_build/\n'
