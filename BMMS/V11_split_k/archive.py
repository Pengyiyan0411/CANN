"""Append R23 and Case15 evidence; preserve the 526 previous archive entries."""
...
def main():
    ...
    subprocess.run([sys.executable,str(ROOT/'V11_shape_dispatch/archive.py'),'--repo',str(repo)],check=True)
    manifest=json.loads(path.read_text(encoding='utf-8'));entries={r['path']:r for r in manifest['files']};sources=[]
    for folder in ['V11_split_k','BMMS_V11_R23']:
        sources.extend(p for p in (ROOT/folder).iterdir() if p.is_file())
    sources.extend(ROOT/p for p in ['BMMS_V11_R23_提交包.zip','audit_current/AUDIT_R22_DELIVERY.md'])
    sources.extend(p for p in (ROOT/'V11_results/2026-09-27_case15_long_k').iterdir() if p.is_file())
    ...
    assert (out/'audit_current/AUDIT_R22_DELIVERY.md').read_bytes()==frozen
    manifest['scope']='Frozen history; R14 baseline; user Case15 long-K diagnosis; R23 Cube Split-K, K1 control and four geometry probes; CANN/NPU validation pending'
