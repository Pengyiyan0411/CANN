...
enum class HardEvent {MTE2_V,V_MTE2,MTE2_S,MTE3_MTE2,V_MTE3,MTE3_V,V_S,S_V};
...
struct TPipe {
    size_t allocated=0;
    template<TPosition P,int D>void InitBuffer(TQue<P,D>& q,int depth,size_t n){Mock::need(depth==1,"shim expects depth one");q.storage=std::make_shared<Mock::Buffer>(n);allocated+=n;Mock::need(allocated<=176*1024,"application UB budget exceeded");}
    template<TPosition P>void InitBuffer(TBuf<P>& b,size_t n){b.storage=std::make_shared<Mock::Buffer>(n);allocated+=n;Mock::need(allocated<=176*1024,"application UB budget exceeded");}
